...
class MalwareDetectionApp(ctk.CTk):
    def __init__(self):
        ...
        # מבנה הרשת של המסך הראשי (Sidebar + Main Content)
    ...
    def show_frame(self, frame_name):
        ...
        frame.tkraise()
        # אם חזרנו לדאשבורד, נרענן את הטבלה
...
class DashboardFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # טבלה פשוטה מבוססת ScrollableFrame
...
class NewScanFrame(ctk.CTkFrame):
    ...
    def select_file(self):
        file_path = filedialog.askopenfilename(title="Select an Executable", filetypes=[("Executable Files", "*.exe *.dll *.sys"), ("All Files", "*.*")])
        if file_path:
...
class StaticAnalysisFrame(ctk.CTkFrame):
    ...
    def _run_static_analysis(self):
        ...
        # עדכון ה-GUI (חייב להתבצע דרך thread ראשי)
    ...
    def _run_dynamic_analysis(self):
        try:
            ...
            if logs:
                ...
                # נציג רק דגימה כדי לא להציף את המסך
                for log in logs[:15]:
